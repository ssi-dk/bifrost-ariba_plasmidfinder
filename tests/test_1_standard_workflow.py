...
from bifrost_ariba_plasmidfinder import launcher as bapl
...
def test_install_component(mydb):
    ...
    args: argparse.Namespace = bapl.parser(["--install"])
    bapl.run_program(args)
...
def test_pipeline(mydb):
    ...
    args = bapl.parser(["-id", sample_id,"-out", "/bifrost_test_data/output/test1"])
    bapl.run_program(args)
    assert os.path.isfile("/bifrost_test_data/output/test1/ariba_plasmidfinder/datadump_complete")